...
export default function SuperAdminPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setError('');
      try {
        const res = await fetch('/api/sa/data', {
  ...
  const handleMoveConfirm = async () => {
    if (!moveDialog || !moveDialog.targetUserId) return;
    setMoving(true);
    setMoveResult(null);
    try {
      ...
      if (res.ok) {
        ...
        setCommunities(prev =>
          prev.map(c =>
            c.communityId === moveDialog.community.communityId
              ? { ...c, ownerId: moveDialog.targetUserId }
              : c
          )
        );
        setTimeout(() => {
  ...
  const filteredCommunities = useMemo(() => {
    ...
    if (selectedOwnerFilter !== 'all') {
      list = list.filter(c => c.ownerId === selectedOwnerFilter);
    }
    if (communitySearch) {
      list = list.filter(c =>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-950">
        <div className="w-full max-w-sm bg-gray-900 border border-gray-800 rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full py-3 rounded-lg bg-purple-700 hover:bg-purple-600 text-white font-semibold transition-colors"
            >
              Unlock
            </button>
          </form>
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100">
      ...
      {error && (
        <div className="m-6 p-4 rounded-lg bg-red-900/30 border border-red-800 text-red-400 text-sm">
          Error: {error}
        </div>
      )}
          ...
          <div className="w-80 flex-shrink-0 border-r border-gray-800 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-left border-b border-gray-800/50 transition-colors ${
              ...
              {filteredUsers.map(user => {
                ...
                return (
                  ...
                  >
                    ...
                    {owned.length > 0 && (
                      <span className="text-xs bg-gray-700 text-gray-300 rounded-full px-2 py-0.5 flex-shrink-0">
                        {owned.length}
                      </span>
                    )}
                  </button>
                    ...
                    >
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {community.tagline && (
                            <p className="text-xs text-gray-600 truncate mt-0.5">{community.tagline}</p>
                          )}
                        </div>
                      </div>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="p-6 border-b border-gray-800">
              ...
              <p className="text-sm text-gray-400 mt-1">
                Transfer ownership of <span className="text-white font-medium">{moveDialog.community.name}</span>
              </p>
            </div>
            ...
            <div className="p-6 pt-0 flex gap-3 justify-end">
              <button
                onClick={() => { setMoveDialog(null); setMoveResult(null); }}
                disabled={moving}
                className="px-4 py-2 rounded-lg border border-gray-700 text-gray-300 hover:bg-gray-800 text-sm transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                onClick={handleMoveConfirm}
                disabled={moving || !moveDialog.targetUserId}
                className="px-4 py-2 rounded-lg bg-purple-700 hover:bg-purple-600 text-white text-sm font-semibold transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
