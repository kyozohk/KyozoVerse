        ...
        <div className="absolute bottom-8 right-8 flex items-center gap-3">
          {ctas.map((cta, index) => (
            <button
              ...
              className={cn(
                "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium h-10 px-4 py-2 transition-all border-2 bg-transparent",
                cta.disabled 
                  ? "opacity-50 cursor-not-allowed" 
                  : "hover:bg-[#E8DFD1] hover:border-[#E8DFD1]"
              )}
              style={{ 
                borderColor: '#E8DFD1', 
                color: cta.disabled ? '#9CA3AF' : '#E8DFD1' 
              }}
              onMouseEnter={(e) => {
                if (!cta.disabled) {
                  e.currentTarget.style.color = '#5B4A3A';
