...
export const TextPostCard: React.FC<TextPostCardProps> = ({ post }) => {
    ...
    // If no image, use the text-specific background. If there is an image, no background is needed for the card itself.
    ...
    return (
        ...
        <div className={`flex ${hasImage ? 'flex-col md:flex-row' : 'flex-col'}`}>
            ...
            <div className={`p-6 flex flex-col justify-between ${hasImage ? 'w-full md:w-2/3 bg-card text-card-foreground' : 'text-black'}`}>
                <div>
